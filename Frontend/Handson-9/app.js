...
const renderCourses = (coursesToRender) => {
    ...
    coursesToRender.forEach(course => {
        ...
        // Accessibility: add tabindex and role button for focus and narration
        article.tabIndex = 0;
    ...
    // Accessibility: Update search results live count status text
    const searchCountEl = document.getElementById('search-count');
    if (searchCountEl) {
        searchCountEl.textContent = `${coursesToRender.length} courses found`;
    }
};
...
// 44. Event Delegation: single click and keydown listener on courseGrid
if (courseGrid) {
    ...
    courseGrid.addEventListener('keydown', (event) => {
        ...
        if (card && (event.key === 'Enter' || event.key === ' ')) {
            event.preventDefault(); // Prevent page scrolls on spacebar keydowns
            handleSelectCourse(card);
        }
    });
    console.log("Registered click and keyboard delegation handler on course-grid.");
}

// Hamburger menu expanded toggle logic
...
if (hamburgerButton && navElement) {
    ...
    console.log("Registered mobile navigation menu toggle listener.");
}